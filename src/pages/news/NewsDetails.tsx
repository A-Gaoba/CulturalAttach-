            ...
            <aside className="mt-12 lg:mt-0 lg:w-1/3">
                ...
                <ul className="space-y-6">
                    {allNews && allNews.map((item) => (
                        <li key={item._id} className="border p-2 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
                            <Link to={`/news/${item._id}`} className="flex items-start gap-4">
                                ...
                                <div>
                                    <h3 className="text-blue-600 hover:text-blue-800 hover:underline font-medium">{item.title}</h3>
                                </div>
                            </Link>
                        </li>
                    ))}
